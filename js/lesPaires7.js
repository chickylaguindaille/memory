...
document.getElementById("btnVoirLivreOr").onclick = function(){	myWindow = window.open('http://localhost/m%C3%A9moire/afficherLivreDOr.php','' ,'');};
document.getElementById("btnEnregistrerDansLivreOr").onclick = function()
	{	
		let lien =  'http://localhost/m%C3%A9moire/saisieLivreDOr.php?nbClics='+nbClics+'&durée='+durée;
		myWindow = window.open(lien,'' ,'');
	};
...
function gérer(unNumCase)
{
	...
	if(nbRectosVus == 1)
	{
		...
		if(numPhoto1 == numPhoto2)
		{
			...
			if(nbTrouvées == 2)
			{
				// masquer les photos afficher les résultats demander pour le livre d'or arrêter le chrono
				document.getElementById("divFille").style.visibility = 'visible';
				document.getElementById("divParent").style.visibility = 'hidden';
				var texte = 'Terminé!! <br> Votre résultat : '+ document.getElementById('info').innerHTML ;
				texte += "<br>Voulez-vous l'enregistrer sur le livre d'or?<br><br>";
				document.getElementById("paragResultat").innerHTML = texte;
				...
				// let message = "vous avez trouvé en "+nbClics+" clics, en "+durée+" secondes.\n Voulez-vous enregistrer votre résultat?";
				// bouclerSurLesPaires.html;
				// alert(message);
				// if (confirm(message)) 
				// { 
					// let nom = prompt('Quel nom?','');
					// message =  "le "+instFin+" "+nom+" "+nbClics+" clics, en "+durée+" secondes";
					// alert("on enregistre "+message);
				// }
...
function enregistrer(unNbClics, uneDurée) 
{
...
function afficherInfos() 
{
	...
	durée = Math.round( (Date.now()-instDébut)/1000);
